...
El sistema implementado corresponde a una arquitectura multiservidor con coordinación distribuida peer-to-peer. Cada nodo ejecuta la lógica de negocio principal, atiende solicitudes de clientes y participa de la coordinación distribuida con los demás nodos del clúster.

Los nodos se configuran mediante un archivo estático nodes.txt, donde se define para cada uno su identificador, host, puerto de atención a clientes y puerto de comunicación peer-to-peer. Esta información es cargada por el módulo RegistroNodos, el cual actúa como mecanismo de membresía estática.

Cada nodo se levanta a través de la clase node.NodoPeer, que integra los siguientes componentes:

ServerImpl: lógica de negocio del sistema.
ClientHandler: atención concurrente de clientes externos.
EscuchadorNodos: recepción de mensajes distribuidos entre nodos.
GestorLatidos: envío y supervisión de heartbeats.
EleccionBully: elección de coordinador distribuido.
RicartAgrawala: exclusión mutua distribuida.
ConsensoBizantino: consenso para la promoción global mediante quórum y tolerancia a fallos bizantinos.
RelojLamport: ordenamiento lógico de eventos.
Las dos funciones principales del sistema que justifican la arquitectura distribuida son la **comparativa de precios regionales** (fuertemente concurrente) y la **compra de juegos** (transacción crítica protegida por un algoritmo de coordinación distribuida de exclusión mutua). Adicionalmente, como función extra, se implementó el establecimiento de una promoción global mediante un consenso simplificado con tolerancia a fallos bizantinos.
...
La transparencia de acceso se logra porque los clientes interactúan con el sistema mediante un mismo protocolo de comunicación basado en sockets TCP y objetos serializados Request y Response, sin importar qué nodo del clúster atienda la solicitud. Desde la perspectiva del cliente, la forma de invocar operaciones como búsqueda de juegos, consulta de precios regionales o compra de juegos se mantiene uniforme, ya que todas siguen la misma estructura de petición y respuesta. Esto permite que el acceso a las funciones del sistema sea consistente y homogéneo, ocultando parcialmente la complejidad interna de la lógica distribuida y de los algoritmos de coordinación implementados entre nodos.
...
Nodos Cliente (Cliente A, B, C...): En esta se representan los dispositivos finales (computadoras de los usuarios) que ejecutan la aplicación cliente (Client.java). Su función es proporcionar la interfaz de consola al usuario, capturar sus peticiones y mostrarlas en pantalla sin realizar procesamiento pesado.

Nodos Peer (Servidores Locales): A diferencia de una arquitectura centralizada clásica, el sistema despliega múltiples máquinas (o instancias de JVM) que ejecutan la clase `node.NodoPeer`. Estos nodos actúan como pares, distribuyendo la carga de los clientes y coordinándose entre sí. Adicionalmente, todos los nodos acceden de manera centralizada al motor MySQL local, asumiendo así el rol de persistencia compartida.
...
Entorno Cliente-Servidor (Red TCP/IP): Los Nodos Cliente se conectan al Nodo Servidor a través de una red TCP/IP, la cuál puede operar dentro de un entorno LAN o a través de la WAN. Esta comunicación se realiza utilizando Sockets, apuntando específicamente al puerto “1009”, en donde por este canal fluyen tanto las peticiones como las respuestas.

Entorno Servidor-Exterior (Red WAN / Internet): Para así poder obtener los datos en tiempo real, el Nodo Servidor debe tener salida a Internet (WAN). Este se comporta como un cliente web que realiza peticiones al Nodo Externo (API de Steam) utilizando los protocolos HTTP (puerto 80) y HTTPS (puerto 443).

Entorno Interno (Localhost / LAN): Dentro del entorno de Servidores Locales, el router distribuye el tráfico externo hacia el Servidor Local. A su vez, la comunicación entre la lógica de Java y la Base de Datos (BD) ocurre de manera interna dentro de la misma máquina física (interfaz de loopback / localhost), utilizando el protocolo JDBC sobre TCP/IP a través del puerto estándar 3306 de MySQL.
...
La capa de atención a clientes está compuesta por el componente ClientHandler, encargado de recibir y gestionar las solicitudes externas provenientes de clientes o del generador de carga mediante conexiones TCP. La capa de lógica de negocio, implementada mediante ServerImpl, procesa las operaciones principales del sistema y actúa como intermediaria entre las solicitudes recibidas, la coordinación entre nodos y el acceso a los datos.

Por otra parte, la capa de coordinación distribuida permite que los nodos se comuniquen entre sí mediante enlaces P2P, con el objetivo de coordinar sus acciones dentro del sistema. En esta capa se integran algoritmos como Lamport, Ricart-Agrawala, Bully, Consenso Bizantino y Heartbeats, los cuales permiten ordenar eventos, controlar el acceso a recursos compartidos, realizar elección de coordinador, acordar el valor de una promoción global y detectar fallos entre nodos.

Finalmente, todos los nodos acceden a una base de datos MySQL compartida, lo que centraliza la persistencia de la información. De esta manera, la arquitectura combina comunicación cliente-servidor para la recepción de solicitudes, comunicación Peer-to-Peer para la coordinación distribuida y una capa de persistencia centralizada para el almacenamiento de datos.
...
En esta capa se concentra la lógica funcional del sistema, es decir, las acciones que determinan cómo se deben gestionar las peticiones, cuándo es necesario acceder a la base de datos y cuándo se requiere coordinar una operación con otros nodos. Por esta razón, ServerImpl actúa como un componente central dentro de cada nodo, conectando la atención de clientes, la coordinación distribuida y la persistencia de datos.

Cuando una solicitud implica el uso de recursos compartidos o requiere mantener consistencia entre nodos, la capa de lógica de negocio se apoya en la capa de coordinación distribuida antes de ejecutar la operación correspondiente. Esto permite que las acciones realizadas por los distintos nodos se mantengan ordenadas y controladas.
...
Dentro de esta capa se implementan distintos algoritmos distribuidos. El algoritmo de Lamport permite establecer un orden lógico entre los eventos generados por los nodos. Ricart-Agrawala se utiliza para controlar el acceso a recursos compartidos, evitando conflictos cuando más de un nodo desea ejecutar una operación crítica. Por su parte, el algoritmo Bully permite realizar la elección de un coordinador cuando sea necesario, especialmente frente a escenarios de falla. El algoritmo de Consenso Bizantino simplificado (PBFT-Lite) se encarga de acordar de forma replicada los valores de configuración global, como la promoción de la tienda. Finalmente, los Heartbeats permiten detectar si un nodo sigue activo mediante el envío periódico de señales entre los participantes del sistema.
...
5.3.1 Diagrama de secuencia para “Compra de Juegos (Transacción con Exclusión Mutua)”
