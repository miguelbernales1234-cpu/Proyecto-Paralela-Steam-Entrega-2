...
7.  Conclusión
Por último, en el desarrollo de esta segunda entrega, el proyecto evolucionó exitosamente de una arquitectura cliente-servidor clásica hacia un verdadero clúster distribuido en topología Peer-to-Peer. Este avance permitió demostrar en la práctica cómo resolver los problemas fundamentales de los sistemas distribuidos. Ante la inherente ausencia de un reloj global físico, la implementación de los Relojes Lógicos de Lamport resultó ser una solución elegante y escalable para ordenar causalmente los eventos en toda la red, garantizando la consistencia transaccional.

Asimismo, se comprobó que es posible mantener la integridad de recursos compartidos sin depender de un cuello de botella centralizado. Mediante el algoritmo de exclusión mutua distribuida de Ricart-Agrawala, el sistema logró procesar operaciones críticas —como la compra de juegos— delegando la responsabilidad de autorización al consenso multicasting del clúster. Las pruebas de tráfico evidenciaron que, si bien esta coordinación P2P introduce un tráfico proporcional al número de nodos, la latencia (p95) y el throughput del sistema se mantuvieron en niveles altamente eficientes para el usuario final.

En materia de tolerancia a fallos, la arquitectura alcanzó un nuevo nivel de resiliencia frente a los escenarios de crash y omisión. La integración de un monitor continuo mediante heartbeats y el algoritmo de elección Bully aseguraron que el sistema no dependiese de un punto único de falla. Tal como se demostró en las pruebas de carga inducida, la caída deliberada de un servidor no detuvo el servicio; el clúster detectó el fallo a los 6 segundos, liberó los bloqueos fantasma y permitió que los nodos sobrevivientes continuaran atendiendo a los clientes.

Finalmente, el análisis de seguridad subraya la necesidad de blindar la capa de transporte en futuras iteraciones. Aunque el diseño lógico es matemáticamente sólido frente a fallos de red, el uso de sockets TCP sin encriptar expone la información a vulnerabilidades de interceptación y deserialización insegura. La adopción de las mitigaciones discutidas, como túneles SSL/TLS, ObjectInputFilters nativos y hasheo fuerte de credenciales, constituiría el paso definitivo para transformar este proyecto académico en un software distribuido preparado para las exigencias y amenazas de un entorno real.
